...
class MainActivity : ComponentActivity() {
                    ...
                    when (currentScreen) {
                        AppScreen.CHAT -> {
                            ChatScreen(
                                ...
                                onOpenCopyright = { currentScreen = AppScreen.COPYRIGHT },
                        ...
                        AppScreen.SETTINGS -> {
                            SettingsScreen(
                                senderId = selfId,
                                onUpdateSenderId = { updateSelfId(it) },
                        ...
                        AppScreen.COPYRIGHT -> {
                            CopyrightScreen(
                                onBackToSettings = { currentScreen = AppScreen.SETTINGS },
                                modifier = Modifier.statusBarsPadding()
                            )
                        }
                    }
    ...
    private fun ensureSelfIdInitialized() {
        ...
        val initialId = if (!savedId.isNullOrBlank()) {
            ...
            UUID.randomUUID().toString().take(8).also { generated ->
                preferences.edit().putString("selfId", generated).apply()
            }
        }
    ...
    private fun updateSelfId(newId: String) {
        val normalized = newId.trim().take(16)
        if (normalized.isBlank()) return
        selfIdState.value = normalized
        preferences.edit().putString("selfId", normalized).apply()
    }
