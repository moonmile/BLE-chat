...
fun SettingsScreen(
    ...
    Scaffold(
        ...
        Column(
            ...
            SettingsItem("送信継続時間",  "${BuildConfig.ADVERTISE_DURATION_SEC} 秒")
            HorizontalDivider(modifier = Modifier.padding(horizontal = 16.dp))
            ...
            SettingsItem("バージョン", "1.0.0")
            HorizontalDivider(modifier = Modifier.padding(horizontal = 16.dp))
            SettingsNavigationItem("著作権情報", onClick = onNavigateToCopyright)
            HorizontalDivider(modifier = Modifier.padding(horizontal = 16.dp))
    ...
    if (showEditDialog) {
        SelfIdEditDialog(
            currentId  = selfId,
            onConfirm  = { newId -> onSelfIdChange(newId); showEditDialog = false },
...
@Composable
private fun SettingsNavigationItem(label: String, onClick: () -> Unit) {
    Row(
        modifier = Modifier
            .fillMaxWidth()
        ...
        text  = {
            Column {
                ...
                OutlinedTextField(
                    ...
                    singleLine   = true,
                    isError      = !isValid,
                    supportingText = { if (!isValid) Text("1〜8文字で入力してください") }
                )
            }
        },
        confirmButton = {
