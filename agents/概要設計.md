...
- チャット画面の各メッセージに ★ アイコンを表示し、タップでお気に入りへ登録・解除できる
- お気に入り一覧画面でお気に入りメッセージを確認できる
- お気に入り画面では、左スワイプで表示されるゴミ箱アイコンをタップしてメッセージを削除できる
- お気に入りはアプリ終了後も永続保存される（SharedPreferences）
...
#### FavoriteRepository
- 責務
	- お気に入りメッセージを `SharedPreferences` に JSON 形式で保存・読み込みする
	- `StateFlow<List<ChatMessage>>` で変更をリアルタイムに通知する
	- `toggle()` でお気に入り追加・解除、`remove()` で個別削除
- 主なメソッド
	- `toggle(message: ChatMessage)` ─ 登録済みなら削除、未登録なら追加
	- `remove(messageId: String)` ─ 指定メッセージを削除
	- `isFavorite(messageId: String): Boolean` ─ 登録済み判定
	- `favorites: StateFlow<List<ChatMessage>>` ─ 現在のお気に入り一覧
...
5. お気に入り機能
	 - `FavoriteRepository` が担当（永続化・StateFlow 通知）
	 - `MessageBubble` の ★ トグルアイコンで登録・解除
	 - `FavoritesScreen` で一覧表示と削除
...
| ★ お気に入りボタン | アンバー色の星アイコン、タップでお気に入り画面へ遷移 | 常時表示 |
| ⚙ 設定ボタン | 設定アイコン、タップで設定画面へ遷移 | 常時表示 |
...
| ★ アイコン位置 | 吹き出しの **左側** | 吹き出しの **右側** |
| ★ アイコン（未登録） | 薄灰色（30% 透過） | 薄灰色（30% 透過） |
| ★ アイコン（登録済み） | アンバー色 `#FFC107` | アンバー色 `#FFC107` |
...
```
NavHost（3画面 + お気に入り画面）
├── ChatScreen（Route.CHAT）
│   ├── ChatHeader(state, onNavigateToSettings, onNavigateToFavorites)
│   │   ├── BleStatusIndicator(canSend: Boolean)
│   │   ├── PeerCountBadge(peerCount: Int)
│   │   ├── ScanningIndicator(isScanning: Boolean)
│   │   ├── IconButton ★（お気に入り画面へ遷移）
│   │   └── IconButton ⚙（設定画面へ遷移）
│   ├── MessageList(messages, selfId, favoriteIds, onToggleFavorite)
│   │   └── MessageBubble(message, isSelf, isFavorite, onToggleFavorite)  // × N
│   └── MessageInputBar(inputText, canSend, onTextChange, onSendClick)
├── SettingsScreen（Route.SETTINGS）
├── CopyrightScreen（Route.COPYRIGHT）
└── FavoritesScreen（Route.FAVORITES）
    └── LazyColumn
